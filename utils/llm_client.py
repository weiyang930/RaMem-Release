...
class LLMClient:
    ...
    def __init__(
        ...
        self.seed = seed if seed is not None else getattr(config, 'LLM_SEED', None)
    ...
    def chat_completion(
        ...
        is_qwen3_model = "qwen3" in (self.model or "").lower()
        ...
        elif is_qwen3_model and not self.enable_thinking:
            # vLLM's OpenAI-compatible server accepts chat_template_kwargs in
            # extra_body. This prevents Qwen3 from spending the whole completion
            # budget on <think>...</think> before returning parseable JSON.
            kwargs["extra_body"] = {

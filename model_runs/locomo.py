"""
LoComo10 Dataset Test for RaMem
...
# Initialize SentenceTransformer model for semantic similarity only when requested.
EVAL_ENABLE_BERT_SCORE = _env_bool("EVAL_ENABLE_BERT_SCORE", True)
EVAL_ENABLE_SBERT_SIMILARITY = _env_bool("EVAL_ENABLE_SBERT_SIMILARITY", True)
EVAL_ENABLE_METEOR = _env_bool("EVAL_ENABLE_METEOR", True)
if EVAL_ENABLE_SBERT_SIMILARITY:
    try:
...
class LoCoMoTester:
    """Test RaMem on LoComo10 dataset."""

    def __init__(self, system: RaMemSystem, dataset_path: str, use_llm_judge: bool = False, test_workers: int = None):
        self.system = system
...
"""
    ...
    def convert_to_dialogues(self, sample: LoCoMoSample) -> List[Dialogue]:
        ...
        for idx, session_id in enumerate(sorted_session_ids):
            session = sample.conversation.sessions[session_id]

            # Parse session_date to ISO 8601
            parsed = dateparser.parse(
                session.date_time,
                settings={'PREFER_DATES_FROM': 'past'}
            ) if session.date_time else None
            session_date_iso = parsed.strftime("%Y-%m-%dT%H:%M:%S") if parsed else session.date_time

            # session_end_date = next session's start date (or far-future sentinel)
            if idx + 1 < len(sorted_session_ids):
                next_session = sample.conversation.sessions[sorted_session_ids[idx + 1]]
                next_parsed = dateparser.parse(
                    next_session.date_time,
                    settings={'PREFER_DATES_FROM': 'past'}
                ) if next_session.date_time else None
                session_end_date_iso = next_parsed.strftime("%Y-%m-%dT%H:%M:%S") if next_parsed else "2099-12-31T00:00:00"
            else:
                session_end_date_iso = "2099-12-31T00:00:00"
            ...
            for turn in session.turns:
                dialogue = Dialogue(
                    ...
                    session_end_date=session_end_date_iso,
                )
    ...
    def _process_single_question(self, qa, qa_idx: int):
        ...
        return {
            ...
            'category': category,
            'temporal_triggered': bool(re.search(r'\b20\d\d\b', question)),
    ...
    def run_test(self, num_samples: int = None, save_results: bool = True, result_file: str = 'locomo10_test_results.json', enable_parallel_questions: bool = False):
        ...
        print("\n" + "="*80)
        print(" RaMem LoComo10 Dataset Test".center(80))
        print("="*80 + "\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test RaMem on LoComo10 dataset')
    ...
    # Create system
    print("Initializing RaMem system...")
    system = RaMemSystem(clear_db=True)

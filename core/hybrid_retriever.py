...
class HybridRetriever:
    ...
    def retrieve(self, query: str, enable_reflection: Optional[bool] = None) -> List[MemoryEntry]:
        ...
        if self.enable_planning:
            results = self._retrieve_with_planning(query, enable_reflection)
        else:
            results = self._semantic_search(query)
        # Expose last retrieved entries for GT rank computation in eval scripts
        config._last_retrieved_contexts = results
        if hasattr(config, "_runtime_local"):
            config._runtime_local.last_retrieved_contexts = results
        return results
    ...
    def _retrieve_with_planning(self, query: str, enable_reflection: Optional[bool] = None) -> List[MemoryEntry]:
        ...
        # Keyword search (Lexical Layer) — kept separate so RRF can fuse with semantic
        keyword_results = self._keyword_search(query, query_analysis)
        print(f"[Keyword Search] Found {len(keyword_results)} results")

        # Structured search (Symbolic Layer) — route by ENABLE_TEMPORAL
        if config.ENABLE_TEMPORAL:
            disable_context_rank = getattr(config, "ABLATE_DISABLE_CONTEXT_AWARE_RANKING", False)
            # Our method: try session-overlap filter (Guard1/2) first.
            # Defer sem+kw merge until we know if SSS fires, so non-triggered
            # questions can use RRF instead of UNION.
            structured_results = self._structured_search(query_analysis, query)
            print(f"[Structured Search] Found {len(structured_results)} results")
            if structured_results:
                if disable_context_rank:
                    # Ablation: keep temporal/contextual candidates but preserve the
                    # original semantic -> lexical -> structured layer order.
                    all_results.extend(keyword_results)
                    combined = all_results + structured_results
                    print("[Planning] Context-aware ranking disabled; temporal hits appended")
                else:
                    # Guard fired — triggered question.
                    # Use UNION for sem+kw pool, prepend SSS results to front.
                    all_results.extend(keyword_results)
                    combined = structured_results + all_results
                    print(f"[Planning] Temporal hits prepended to context front")
            else:
                # Guard 1/2 didn't fire — non-triggered question.
                # Use RRF(sem, kw) for semantic/lexical fusion.
                # Also run metadata fallback search for person/entity/location.
                if disable_context_rank:
                    all_results.extend(keyword_results)
                    rrf_sem_kw = all_results
                    print("[Keyword Search] Context-aware ranking disabled; using UNION")
                else:
                    rrf_sem_kw = self._rrf_merge(all_results, keyword_results,
                                                 top_k=self.semantic_top_k)
                    print(f"[Keyword Search] RRF-fused with semantic → {len(rrf_sem_kw)} results")
                struct_fallback = self._original_structured_search(query_analysis)
                if struct_fallback:
                    print(f"[Structured Search] Guard fallback: {len(struct_fallback)} results")
                combined = list(rrf_sem_kw) + struct_fallback
        else:
            # Non-temporal retrieval path.
            if getattr(config, 'USE_RRF_BASELINE', False):
                # RRF(sem_variants, bm25_kw): better fusion than flat UNION.
                # all_results = merged semantic variants; keyword_results = BM25 list.
                rrf_sem_kw = self._rrf_merge(all_results, keyword_results,
                                             top_k=self.semantic_top_k)
                print(f"[Keyword Search] RRF-fused with semantic → {len(rrf_sem_kw)} results")
                structured_results = self._original_structured_search(query_analysis)
                print(f"[Structured Search] Found {len(structured_results)} results")
                combined = list(rrf_sem_kw) + structured_results  # structured appended last
            else:
                # Preserve semantic -> lexical -> metadata ordering.
                all_results.extend(keyword_results)
                structured_results = self._original_structured_search(query_analysis)
                print(f"[Structured Search] Found {len(structured_results)} results")
                combined = all_results + structured_results  # append last
        merged_results = self._merge_and_deduplicate_entries(combined)
    ...
    def _analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        When ENABLE_TEMPORAL is True and CONV_DATE_START/END are set, the prompt
        is augmented in two ways:
          1. Injects the conversation date window as context.
          2. Adds a firm "always return dates" instruction so the LLM does not
             escape with null for implicit time references (e.g. "last August",
             "this summer").  For queries with an explicit year the instruction
             is redundant but harmless — the year anchors extraction regardless.
        When ENABLE_TEMPORAL is False or dates are not set, the prompt has no
        conversation window and no firm date instruction.
        """
        conv_ctx = ""
        if (config.ENABLE_TEMPORAL
                and getattr(config, 'CONV_DATE_START', '')
                and getattr(config, 'CONV_DATE_END', '')):
            conv_ctx = (
                f"Conversation context: This conversation took place between "
                f"{config.CONV_DATE_START} and {config.CONV_DATE_END}. "
                f"If the query uses relative or implicit time references "
                f"(e.g. \"August\", \"last summer\", \"the weekend before\"), "
                f"resolve them using this date window.\n"
                f"A temporal signal is confirmed in this query. "
                f"Extract start and end dates for any time period mentioned. "
                f"Always return start and end dates — do not return null.\n\n"
            )

        prompt = f"""{conv_ctx}Analyze the following query and extract key information:
...
6. time_range_start: If a time period is mentioned, the ISO 8601 start date (YYYY-MM-DD). For "last two weeks of March 2022" this is "2022-03-17". For "first weekend of May 2021" this is "2021-05-01". Null if no time period.
7. time_range_end: The ISO 8601 end date for the time period. For "last two weeks of March 2022" this is "2022-03-31". For "first weekend of May 2021" this is "2021-05-02". Null if no time period.
...
"""
    ...
    def _original_structured_search(self, query_analysis: Dict[str, Any]) -> List[MemoryEntry]:
        """
        Metadata structured search.
        Filters by persons/location/entities/timestamp extracted from LLM query analysis.
        Uses dateparser to parse time_expression → timestamp range on the per-entry
        `timestamp` field (not session_date).
        Called when temporal session filtering is not active. Results are appended
    ...
    @staticmethod
    def _rrf_merge(sem_list: List[MemoryEntry], kw_list: List[MemoryEntry],
                   k: int = 60, top_k: int = 25) -> List[MemoryEntry]:
        """Reciprocal Rank Fusion of two ranked lists."""
        scores: dict = {}
        order: list  = []
        for rank, entry in enumerate(sem_list, 1):
            eid = entry.entry_id
            if eid not in scores:
                scores[eid] = 0.0
                order.append(entry)
            scores[eid] += 1.0 / (k + rank)
        for rank, entry in enumerate(kw_list, 1):
            eid = entry.entry_id
            if eid not in scores:
                scores[eid] = 0.0
                order.append(entry)
            scores[eid] += 1.0 / (k + rank)
        order.sort(key=lambda e: -scores[e.entry_id])
        return order[:top_k]

    @staticmethod
    def _prox_rerank(entries: List[MemoryEntry], win_start: str, win_end: str,
                     top_k: int = 25) -> List[MemoryEntry]:
        """Sort entries so those whose session overlaps the Guard2 window rank first.
        Within the overlap group, preserves original order (stable sort on score desc).
        Entries outside the window decay over 30 days; stable tie-break on original index.
        """
        ...
        def _score(entry, orig_idx):
            s = _parse(entry.session_date or "")
            e = _parse((entry.session_end_date or entry.session_date) or "")
            if s is None:
                return (0.0, orig_idx)
            if e is None:
                e = s
            latest_start  = max(s, ws)
            earliest_end  = min(e, we)
            if earliest_end >= latest_start:
                return (1.0, orig_idx)
            gap = (latest_start - earliest_end).days
            return (max(0.0, 1.0 - gap / 30.0), orig_idx)

        scored = [(_score(e, i), e) for i, e in enumerate(entries)]
        scored.sort(key=lambda x: (-x[0][0], x[0][1]))
        return [e for _, e in scored][:top_k]

    def _structured_search(self, query_analysis: Dict[str, Any], query: str = None) -> List[MemoryEntry]:
        """
        Symbolic Layer Retrieval (Section 3.3)
        R_sym = Top-n({m_i | Meta(m_i) ⊨ q_sym})

        When a time expression is present, uses session span overlap as a hard
        filter to return all entries whose session interval overlaps the query window.
        Fires when the raw query contains a temporal anchor (year, month name, season,
        or week/weekend reference), preventing hallucinated date ranges for queries
        with no real time anchor.
        Guard 1 mode is controlled by config.GUARD1_EXTENDED:
          True  → extended regex (year OR month OR season OR week/weekend)
          False → year-only fallback (\\b20\\d\\d\\b).
        """
        # Guard 0: global temporal toggle.
        if not config.ENABLE_TEMPORAL:
            return []

        # Guard 1: temporal signal must appear in the raw query string (deterministic)
        # Extended: year OR month name OR season OR week/weekend reference
        # Fallback:  year only (\b20\d\d\b) — flip config.GUARD1_EXTENDED to revert
        _GUARD1_YEAR_RE = re.compile(r'\b20\d\d\b')
        ...
        _guard1_re = _GUARD1_EXT_RE if config.GUARD1_EXTENDED else _GUARD1_YEAR_RE
        if not getattr(config, "ABLATE_DISABLE_CUE_GUARD", False) and not _guard1_re.search(query or ''):
            return []
        if getattr(config, "ABLATE_DISABLE_CUE_GUARD", False):
            print("[Structured Search] Cue guard disabled; using LLM time_range if available")

        # Guard 2: LLM must have extracted both start and end dates
        time_start = query_analysis.get("time_range_start")
        time_end   = query_analysis.get("time_range_end")

        if not (time_start and time_end):
            # Guard 2 returned null — even with the firm "always return" instruction
            # the LLM could not resolve a date range.  This means the temporal
            # signal is genuinely ambiguous (e.g. "the summers" across multiple
            # years).  Do NOT guess: fall back to pure semantic search by returning
            # no structured results.  The semantic + keyword layers still run.
            return []

        # Side-channel: record the extracted window so eval scripts can inspect it.
        # Reset to None before each call (done in eval scripts); set here on success.
        config._guard2_window = f"{time_start}/{time_end}"
        if hasattr(config, "_runtime_local"):
            config._runtime_local.guard2_window = config._guard2_window

        # Optional session buffer: accounts for the lag between the event date
        # (what G2 extracts) and the session date in the DB (when it was discussed).
        # Conversations reference past events; the session may be recorded a few
        # days after the event.  Buffer is applied symmetrically.
        buffer_days = getattr(config, 'TEMPORAL_WINDOW_BUFFER_DAYS', 0)
        if buffer_days:
            try:
                from datetime import date as _date, timedelta as _td
                _ts = _date.fromisoformat(time_start[:10]) - _td(days=buffer_days)
                _te = _date.fromisoformat(time_end[:10])   + _td(days=buffer_days)
                time_start = _ts.isoformat()
                time_end   = _te.isoformat()
                print(f"[Structured Search] Buffer ±{buffer_days}d applied → "
                      f"{config._guard2_window}  →  {time_start}/{time_end}")
            except Exception:
                pass   # malformed date — keep original

        # Side-channel: record the BUFFERED window actually used for filtering.
        # Diagnostics can compare _guard2_window (raw LLM) vs _guard2_buffered_window (used).
        config._guard2_buffered_window = f"{time_start}/{time_end}"
        if hasattr(config, "_runtime_local"):
            config._runtime_local.guard2_buffered_window = config._guard2_buffered_window

        start_iso = time_start if "T" in time_start else f"{time_start}T00:00:00"
        end_iso   = time_end   if "T" in time_end   else f"{time_end}T23:59:59"
        print(f"[Structured Search] Session overlap range: {time_start} → {time_end}")

        # Temporal-filtered ranking: session overlap filter + ranker selection.
        # SSS_USE_RRF=True  → RRF(sem_filter + BM25_filter) optionally + proximity rerank
        # SSS_USE_BM25=True → keyword_search_with_filter (BM25/FTS only, legacy)
        # default           → semantic_search_with_filter (cosine similarity, original)
        overlap_filter = f"session_date <= '{end_iso}' AND session_end_date >= '{start_iso}'"

        if getattr(config, "SSS_USE_RRF", False):
            # RRF: merge semantic-filtered + BM25-filtered lists within the session window
            ana_keywords = query_analysis.get("keywords", []) or query.split()
            print(f"[Structured Search] Using RRF(sem+BM25) filter={overlap_filter}, top_k={self.structured_top_k}")
            sem_res = self.vector_store.semantic_search_with_filter(
                query=query,
                where_clause=overlap_filter,
                top_k=self.structured_top_k
            )
            kw_res = self.vector_store.keyword_search_with_filter(
                keywords=ana_keywords,
                where_clause=overlap_filter,
                top_k=self.structured_top_k
            )
            merged = self._rrf_merge(sem_res, kw_res, top_k=self.structured_top_k)
            if (getattr(config, "SSS_ENABLE_PROX_RERANK", False)
                    and not getattr(config, "ABLATE_DISABLE_CONTEXT_AWARE_RANKING", False)):
                print(f"[Structured Search] Proximity rerank applied")
                merged = self._prox_rerank(merged, time_start, time_end, top_k=self.structured_top_k)
            return merged

        elif getattr(config, "SSS_USE_BM25", False):
            keywords = query.split()
            print(f"[Structured Search] Using keyword_search_with_filter (BM25), filter={overlap_filter}, top_k={self.structured_top_k}")
            return self.vector_store.keyword_search_with_filter(
                keywords=keywords,
                where_clause=overlap_filter,
                top_k=self.structured_top_k
            )
            ...
            print(f"[Structured Search] Using semantic_search_with_filter, filter={overlap_filter}, top_k={self.structured_top_k}")
            sem_res = self.vector_store.semantic_search_with_filter(
                query=query,
                where_clause=overlap_filter,
                top_k=self.structured_top_k
            ...
            if (getattr(config, "SSS_ENABLE_PROX_RERANK", False)
                    and not getattr(config, "ABLATE_DISABLE_CONTEXT_AWARE_RANKING", False)):
                print(f"[Structured Search] Proximity rerank applied")
                sem_res = self._prox_rerank(sem_res, time_start, time_end, top_k=self.structured_top_k)
            return sem_res
